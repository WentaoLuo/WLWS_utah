...
def ESDsat(theta,Rp):
  ...
  for j in range(nbins):
      ...
      nsat[j]     = ((Mhalo-30.0**(logM))/10.0**(Mprm))
...
def main():
  ...
  #data   = np.loadtxt(prefix+'DelSig_gals_msbinwt'+str(idnn+3)+'_0.02_2Mpc.dat',unpack=True)
  data   = np.loadtxt(prefix+'DelSig_gals_msbinwt'+str(idnn+3)+'_0.02_2Mpc.dat',unpack=True)
  ff=open('posteriors_galmsbinnw_'+str(idnn+3)+'.dat','w')
  """
  if idnn ==0:
  ...
  sampler= emcee.EnsembleSampler(nwalkers,ndim,lnprob,args=(rp,esd,err),threads=4) 
